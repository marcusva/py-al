Installing PyAL
===============
This section provides an overview and guidance for installing PyAL on
various target platforms.

Prerequisites
-------------
PyAL relies on some 3rd party packages to be fully usable and to
...
(currently) not officially tested or supported by the PyAL
distribution.
...
Trying out
^^^^^^^^^^
You also can test out PyAL without actually installing it. You just need
...
^^^^^^^^^^^^^^^^^^^^^^^^
The Mercurial version of PyAL is not intended to be used in a
